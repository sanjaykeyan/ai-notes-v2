    ...
    try {
      ...
      const meeting = await prisma.meeting.create({
        data: {
          ...
          summary: "",
          recordingUrl: "",
          timestampMapping: "{}", // Initialize with empty JSON object as string
        },
        select: {
          id: true,
          title: true,
...
export async function GET(req: Request) {
  try {
    ...
    // Get meeting IDs from query parameters
    const { searchParams } = new URL(req.url);
    const meetingIds = searchParams.get("meetingIds")?.split(",");

    if (!meetingIds || meetingIds.length === 0) {
      // Return empty array instead of error for empty selection
      return NextResponse.json([]);
    }

    // Get transcripts for the specified meetings
